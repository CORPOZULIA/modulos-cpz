...
class Usuarios extends Controller
{
    ...
    public function crear($request, $info)
    {
        ...
        else if($persona[0]->empleado->user != null && $persona[0]->empleado->user->edo_reg == 0 && $this->_auth('CREATE'))
        {
            ...
            $persona[0]->empleado->user->password = $request->input('password');
            $persona[0]->empleado->user->save();
    ...
    private function _auth($accion)
    {
        ...
        if(verificar_permisos('CREATE', $this->modulo_id))
        {
            push_auditoria('El usuario ejercio una accion de '.$accion, 'Usuarios', 'usuarios');
            return true;
        }
